...
import { validateDiscountCode as validateCodeService } from '../services/discountCodes.service';
...
export const useDiscountCode = (orderTotal = 0) => {
    ...
    const validate = useCallback(
        async (codeToValidate = code) => {
            ...
            if (!isValid) {
                setError(
                    'Invalid code format. Code must be 5 alphanumeric characters.',
                );
                return false;
            }
            ...
            try {
                ...
                const result = await validateCodeService(
                    codeToValidate,
                    orderTotal,
                );
...
export const useDiscountCodes = (availableOnly = true) => {
    ...
    const loadCodes = useCallback(async () => {
        try {
            ...
            const { getAllDiscountCodes } = await import(
                '../services/discountCodes.service'
            );
...
export const useDiscountCodeDetails = (codeString) => {
    ...
    const loadCode = useCallback(async () => {
        ...
        try {
            ...
            // Import dynamically to avoid circular dependencies
            const { getDiscountCodeById } = await import(
                '../services/discountCodes.service'
            );
