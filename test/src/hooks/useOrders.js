  ...
  const fetchMyOrders = useCallback(async () => {
    ...
    const fetchAllOrders = useCallback(async () => {
        try {
            ...
            if (response.success && response.orders) {
                ...
                throw new Error(
                    response.message || 'Failed to fetch all orders',
                );
            }
        } catch (err) {
            const errorMsg =
        ...
        async (checkoutData) => {
            try {
                ...
                if (response.success && response.data) {
                    ...
                    throw new Error(
                        response.message || 'Failed to checkout from cart',
                    );
                }
            } catch (err) {
                const errorMsg =
    ...
    const orderStats = {
        ...
        totalSpent: orders.reduce((sum, order) => {
            const amount =
                order.totalAmount?.$numberDecimal || order.totalAmount || 0;
