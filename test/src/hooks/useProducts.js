        ...
        async (productData) => {
            try {
                ...
                const productPayload = {
                    ...
                    categoryId: Array.isArray(productData.categoryId)
                ...
                if (variants && variants.length > 0) {
                    ...
                    if (variantIds.length > 0) {
                        await productsService.patchProduct(newProduct._id, {
                            variants: variantIds,
                        });
                    }
                }
    ...
    const updateProduct = useCallback(
        async (id, productData) => {
            try {
                ...
                const updated = await productsService.patchProduct(
                    id,
                    updatePayload,
                );
        ...
        const fetchAll = async () => {
            try {
                ...
                if (categories && categories.length > 0) {
                    categories.forEach(
                        ({ key, categoryId, category, ...params }) => {
                            keys.push(key);
