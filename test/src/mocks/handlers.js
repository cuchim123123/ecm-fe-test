...
export const handlers = [
    ...
    http.get(`${API_BASE_URL}/users`, ({ request }) => {
        ...
        console.log('[Mock Handler] Users request:', {
            role,
            isVerified,
            socialProvider,
            search,
        ...
        if (isVerified && isVerified !== 'all') {
            ...
            users = users.filter(
                (u) => Boolean(u.isVerified) === isVerifiedBool,
            );
        }
        ...
        if (socialProvider && socialProvider !== 'all') {
            if (socialProvider === 'local') {
                users = users.filter(
                    (u) => !u.socialProvider || u.socialProvider === 'local',
                );
            } else {
                users = users.filter(
                    (u) => u.socialProvider === socialProvider,
                );
            }
        }
    ...
    http.get(`${API_BASE_URL}/products`, ({ request }) => {
        ...
        console.log('[Mock Handler] Products request:', {
            page,
            limit,
            url: request.url,
        });
        ...
        const sortParam =
            url.searchParams.get('sort') || url.searchParams.get('sortBy');
        ...
        if (minPrice > 0 || maxPrice !== Infinity) {
            products = products.filter((p) => {
                ...
                return (
                    productMinPrice <= maxPrice && productMaxPrice >= minPrice
                );
            });
        }
        ...
        if (minRating > 0) {
            products = products.filter(
                (p) => (p.averageRating || p.rating || 0) >= minRating,
            );
        }
    ...
    http.patch(
        ...
        async ({ params, request }) => {
            ...
            const reviewIndex = localReviews.findIndex(
                (r) => r._id === reviewId,
            );
            ...
            if (
                ...
                updateData.content.trim().length === 0
            ) {
                return HttpResponse.json(
                    {
                        success: false,
    ...
    http.get(`${API_BASE_URL}/users/:userId/reviews`, ({ params, request }) => {
        ...
        const userLocalReviews = localReviews.filter(
            (r) => r.userId === userId,
        );
    ...
    http.post(`${API_BASE_URL}/cart`, async ({ request }) => {
        ...
        return HttpResponse.json(
            cart[existingItemIndex] || cart[cart.length - 1],
            {
                status: 201,
            },
        );
        ...
        async ({ request }) => {
            ...
            return HttpResponse.json({
                ...
                discountCode: {
                    ...
                    remainingUses:
                        validation.discountCode.usageLimit -
                        validation.discountCode.usedCount,
    ...
    http.post(
        `${API_BASE_URL}/discount-codes/:codeId/use`,
        async ({ params }) => {
            const { codeId } = params;

            // Get discount codes from localStorage (to track usage)
            const localCodes = JSON.parse(
                localStorage.getItem('discountCodes') || '[]',
            );

            // Find in mock data
            let discountCode = MOCK_DISCOUNT_CODES.find(
                (dc) => dc._id === codeId,
            );
