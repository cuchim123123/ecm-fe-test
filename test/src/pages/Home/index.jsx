...
const HeroSection = lazy(() =>
    import('./components/Hero').then((m) => ({ default: m.HeroSection })),
);
const ProductShowcaseSection = lazy(() =>
    import('./components/Category').then((m) => ({
        default: m.ProductShowcaseSection,
    })),
);
const CategorizedProductsSection = lazy(() =>
    import('./components/Category').then((m) => ({
        default: m.CategorizedProductsSection,
    })),
);
...
const Home = () => {
    ...
    return (
        <div className="min-h-screen bg-white overflow-x-hidden">
            ...
            ) : categorizedProducts.featured &&
              categorizedProducts.featured.length > 0 ? (
                <Suspense
                    fallback={
                ...
                >
                    <HeroSection
                        featuredProducts={categorizedProducts.featured}
                    />
                </Suspense>
            ) : null}
                    ...
                    <div className="h-[500px] bg-white animate-pulse px-4 sm:px-6 md:px-[5%] py-12 sm:py-16 md:py-20">
                        ...
                        <div className="flex gap-4">
                            {Array.from({ length: 4 }).map((_, i) => (
                                <div
                                    key={i}
                                    className="w-[200px] sm:w-[220px] md:w-[280px] h-[340px] sm:h-[360px] md:h-[380px] bg-gray-200 rounded-lg"
                                ></div>
                            ))}
                        </div>
                    </div>
