...
const ProductVariantSelector = lazy(
    () => import('./components/ProductVariantSelector'),
);
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            addItem(product._id, quantity, selectedVariant._id)
                .then(() => {
                    console.log(
                        '[PRODUCT DETAIL] Background API call completed in',
                        (performance.now() - apiStartTime).toFixed(0),
                        'ms',
                    );
                ...
                .catch((err) => {
                    console.error(
                        '[PRODUCT DETAIL] Background API failed:',
                        err,
                    );
            ...
            console.log(
                '[PRODUCT DETAIL] Toast shown in:',
                (performance.now() - startTime).toFixed(0),
                'ms',
            );
        } catch (err) {
            console.error(
                '[PRODUCT DETAIL] Error after',
                (performance.now() - startTime).toFixed(0),
                'ms:',
                err,
            );
