...
const ProductInfo = ({ product, rating, reviewCount, price, originalPrice, inStock, stock, discount, selectedVariant }) => {
  return (
    <div className="flex flex-col gap-4">
      {/* Title & Brand */}
      <h1 className="text-3xl font-bold text-slate-800 leading-tight m-0">{product.name}</h1>
      {product.brand && (
        ...
        {selectedVariant ? (
          <>
            <div className="flex items-center gap-3">
              <span className="text-4xl ">{formatPrice(price)}</span>
          ...
          <div className="flex items-center gap-3">
            <span className="text-4xl font-bold text-gray-400">Select all options to see price</span>
          </div>
        )}
      </div>
      ...
      {product.shortDescription && (
        <p className="text-base text-slate-600 leading-relaxed">{product.shortDescription}</p>
      )}
