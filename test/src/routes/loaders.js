...
export async function homeLoader() {
    try {
        ...
        const [featuredResponse, newProductsResponse] = await Promise.all([
            // Featured products for hero carousel (critical)
            dedupedFetch('featured-6', () =>
        ...
        const featuredProducts =
            featuredResponse.products || featuredResponse || [];
        const newProducts =
            newProductsResponse.products || newProductsResponse || [];
    ...
    try {
        ...
        if (!product) {
            throw new Response('Product not found', {
                status: 404,
                statusText: 'The product you are looking for does not exist',
            });
        }
