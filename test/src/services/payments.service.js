...
export const customerConfirmVietQR = async (orderId) => {
    const response = await apiClient.post(
        `/payments/vietqr/customer-confirm/${orderId}`,
    );
...
export const adminConfirmVietQR = async (orderId) => {
    const response = await apiClient.post(
        `/payments/vietqr/admin/${orderId}/confirm`,
    );
    return response.data;
};
...
export const adminRejectVietQR = async (orderId) => {
    const response = await apiClient.post(
        `/payments/vietqr/admin/${orderId}/reject`,
    );
    return response.data;
};
...
export const zaloPayReturn = async (params) => {
    const response = await apiClient.get('/payments/zalopay/return', {
        params,
    });
    return response;
};
